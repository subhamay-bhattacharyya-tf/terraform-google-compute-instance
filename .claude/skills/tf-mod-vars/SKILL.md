...
| `zone` | `string` | `us-central1-a`, `us-central1-b`, `us-east1-b`, etc. |
| `machine_type` | `string` | `e2-micro`, `e2-small`, `n1-standard-2`, `n2-standard-4`, etc. |
...
### Example: Google Compute Instance Module

```hcl
variable "compute_instance_config" {
  description = "Configuration for the Google Compute Instance."
  type = object({
    base_name    = string
  ...
  validation {
    condition     = length(var.compute_instance_config.base_name) > 0 && length(var.compute_instance_config.base_name) <= 30
  ...
  validation {
    ...
    ], var.compute_instance_config.boot_disk.type)
